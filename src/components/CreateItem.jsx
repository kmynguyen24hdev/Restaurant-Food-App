...
const CreateItem = () => {
  ...
  const saveDetails = () => {
    setIsLoading(true);
    try {
      if (!title || !categories || !imageAssets || !price || !category) {
        setFields(true);
        setMsg("Required fields can't between");
        setAlertStatus("danger");
        setTimeout(() => {
          setFields(false);
          setIsLoading(false);
          ...
          >
            ...
            {categories &&
              categories.map((item) => (
                <option
                  key={item.id}
                  className="textb-base border-0 outline-none capitalize bg-white text-headingColor"
